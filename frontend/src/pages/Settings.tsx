    ...
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        ...
        <div className="card">
          ...
          {profileMsg.text && (
            <div className={`p-3 mb-4 rounded text-sm ${profileMsg.type === 'error' ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'}`}>
              {profileMsg.text}
            </div>
          )}
          <div className="space-y-4">
            <div>
              ...
              <input 
                type="text" 
                className={`input ${!isAdmin ? 'bg-gray-100' : ''}`} 
                value={name} 
                onChange={(e) => setName(e.target.value)}
                disabled={!isAdmin} 
              />
              {!isAdmin && <p className="text-xs text-gray-500 mt-1">Le nom complet ne peut pas être modifié par un membre.</p>}
            </div>
            <div>
        ...
        <div className="card">
          ...
          {securityMsg.text && (
            <div className={`p-3 mb-4 rounded text-sm ${securityMsg.type === 'error' ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'}`}>
              {securityMsg.text}
            </div>
          )}
          <div className="space-y-4">
        ...
        {/* Administration: Gestion des membres (Uniquement pour les Administrateurs) */}
        {isAdmin && (
          <div className="card border-l-4 border-l-purple-600">
            <div className="flex items-center gap-3 mb-4">
              <Shield className="text-purple-600" size={24} />
              <h2 className="text-xl font-bold text-gray-900">Administration - Gestion des Membres</h2>
            </div>
            <p className="text-sm text-gray-600 mb-4">En tant qu'administrateur, vous pouvez gérer les membres de l'application. Vous ne pouvez pas supprimer un autre administrateur.</p>
            
            {membersMsg.text && (
              <div className={`p-3 mb-4 rounded text-sm ${membersMsg.type === 'error' ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'}`}>
                {membersMsg.text}
              </div>
            )}
              ...
              {members.map((member) => (
                <div key={member.id} className="flex items-center justify-between p-3 border rounded-lg bg-gray-50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-semibold text-gray-900 leading-tight">{member.name} {member.id === user?.id && <span className="text-xs text-primary-600 font-normal ml-1">(Vous)</span>}</h4>
                      <p className="text-xs text-gray-500">{member.email}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <span className={`px-2 py-1 text-xs rounded-full font-medium ${member.role === 'Administrateur' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}>
                      {member.role}
                    </span>
                    <button 
                      onClick={() => handleDeleteMember(member.id, member.name)}
                      disabled={member.role === 'Administrateur'}
                      className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded transition-colors disabled:opacity-30 disabled:hover:text-gray-400 disabled:hover:bg-transparent"
        ...
        {/* Notifications (Placeholder visuel tel que demandé) */}
          ...
          <div className="space-y-3 pointer-events-none">
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <div>
                ...
                <div className="text-sm text-gray-600">Recevoir des notifications par email pour les mises à jour</div>
              </div>
            </label>
            <label className="flex items-center gap-3 cursor-pointer">
              <input type="checkbox" className="w-5 h-5 text-primary-600 rounded" defaultChecked />
              <div>
                ...
                <div className="text-sm text-gray-600">Être notifié quand une tâche vous est assignée</div>
              </div>
            </label>
          </div>
        </div>
        
        {/* Apparence (Placeholder visuel tel que demandé) */}
        <div className="card opacity-60">
          <div className="flex items-center gap-3 mb-4">
